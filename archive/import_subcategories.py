...
class SubcategoryImporter:
    ...
    def generate_ingredient_name(self, category_name: str, subcategory_name: str) -> str:
        """
        Генерирует имя ингредиента по формуле: "Категория " + подкатегория.lower().
        ...
        Returns:
            Сгенерированное имя ингредиента
        """
        ingredient_name = f"{category_name} {subcategory_name.lower()}"
        
        # Проверяем длину (ограничение Django CharField 255 символов)
        if len(ingredient_name) > 255:
            logger.warning(f"Слишком длинное имя '{ingredient_name}' - усекаем до 255 символов")
    ...
    def process_subcategory(self, subcategory: Dict) -> bool:
        ...
        try:
            ...
            ingredient_name = self.generate_ingredient_name(category_name, subcategory['name'])
